...
- [x] 2026-09-22 : `Code.gs` v4 déployé et vérifié (GET → `v:4`). Raccourci « Muscu Pas » créé par Mehdi et testé : réponse `{ok, v:4, steps:{'2026-09-22':{n:2383,src:'sante'}}}`. Piège rencontré : le champ `steps` de « Obtenir le contenu de l'URL » pointait sur les *Échantillons de Santé* (le raccourci demandait « envoyer 2 échantillons ») au lieu de la variable *Statistiques* (Somme). Sources dans Santé : « iPhone de Mehdi », « Apple Watch de Mehdi », « Apple Watch Series 7 (GPS) » (ancien jumelage). Sans filtre le raccourci additionne tout (2 383 vs 1 590 dans Santé) ; iPhone seul = 961 ; **montre seule (« Apple Watch de Mehdi ») = 1 520 ≈ Santé → filtre retenu**. Raccourcis ne sait pas refaire la fusion de Santé ; Pedometer++ n'a pas été vérifié comme source d'actions Raccourcis.
