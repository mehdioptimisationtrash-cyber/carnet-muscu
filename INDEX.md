...
- Proposition (`Coach.restFor`) : polyarticulaire 2:00 (1:30–3:00), isolation 1:15 (1:00–2:00), jamais < 60 s ; ressenti de la série : 😄 −30 s, 😣 +30 s (le chrono en cours est ajusté au choix du smiley, `adjustRest`) ; + `e.restAdj` appris par exo (±20 s/séance, borné −45/+60) : repos respecté mais série suivante à fond/ratée → +10 ; série suivante facile → −10 ; parti avant la fin et ça passe → −5. Type de mouvement deviné (`POLY_RE`) ou choisi dans ⋯ (`e.restKind`). ⚙︎ Réglages : « Automatique (coach) » (défaut, `settings.restAuto`) ou fixe.
- Sources citées dans le code : Schoenfeld 2016, Grgic 2018, Singer 2024 (bénéfice au-delà de ~90 s quasi nul pour l'hypertrophie).
- Bilan : « ⏱️ Repos : réel vs proposé » (+ minutes perdues) et conseils `Coach.restAdvice` ; Stats → Axes idem ; `history.exos[id].rest = {prop, real}`.
...
- [x] 2026-09-22 : `Code.gs` v4 déployé et vérifié (GET → `v:4`). Raccourci « Muscu Pas » créé par Mehdi et testé : réponse `{ok, v:4, steps:{'2026-09-22':{n:2383,src:'sante'}}}`. Piège rencontré : le champ `steps` de « Obtenir le contenu de l'URL » pointait sur les *Échantillons de Santé* (le raccourci demandait « envoyer 2 échantillons ») au lieu de la variable *Statistiques* (Somme). Sources dans Santé : « iPhone de Mehdi », « Apple Watch de Mehdi », « Apple Watch Series 7 (GPS) » (ancien jumelage). Sans filtre le raccourci additionne tout (2 383 vs 1 590 dans Santé) ; iPhone seul = 961 ; montre seule = 1 520 ≈ Santé. Mehdi ne porte pas la montre tous les jours → **`Code.gs` v5 : le raccourci envoie `{iphone, montre}` et le script garde le max** (`detail` dans la colonne source). Raccourcis ne sait pas refaire la fusion de Santé ; Pedometer++ n'a pas été vérifié comme source d'actions Raccourcis. Test unitaire `tools/test_codegs.js`.
